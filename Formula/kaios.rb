class Kaios < Formula
  ...
  url "https://github.com/morning-verlu/KAI/releases/download/v0.1.84/kaios-0.1.84.tar"
  sha256 "73438534b930e9d0de1cfdb80d10d28cff192ecfcd11c3d7dfc6f87fd9ed83a6"
  license "Apache-2.0"
  ...
  test do
    assert_match "kaios 0.1.84", shell_output("#{bin}/kaios --version")
    ...
    assert_match "kaios quickstart", doctor
    assert_match "kaios demo", doctor
    assert_match "kaios doctor --fix --dry-run --ci", doctor
    assert_match "kaios doctor --fix --ci", doctor
    ...
    assert_match '"id": "quickstart"', doctor_json
    assert_match '"command": "kaios quickstart"', doctor_json
    assert_match '"id": "repair-project"', doctor_json
    assert_match '"command": "kaios doctor --fix --dry-run --ci"', doctor_json
    assert_match '"id": "setup-project"', doctor_json
    assert_match '"command": "kaios setup --ci"', doctor_json
    next_report = shell_output("#{bin}/kaios next")
    assert_match "KAI OS next", next_report
    assert_match "schema: kaios.next/v1", next_report
    assert_match "status: repair", next_report
    assert_match "command: kaios doctor --fix --dry-run --ci", next_report
    assert_match "fix_first: kaios doctor --fix --dry-run --ci", next_report
    next_json = shell_output("#{bin}/kaios next --json")
    assert_match '"schema": "kaios.next/v1"', next_json
    assert_match '"status": "repair"', next_json
    assert_match '"id": "repair-project"', next_json
    assert_match '"command": "kaios doctor --fix --dry-run --ci"', next_json
    ...
    refute_match "kaios gate --config kaios.json", missing_verify
    missing_custom_verify = shell_output(
      "cd missing-config-fixture && #{bin}/kaios gate --config workflows/research.json 2>&1",
      2,
    )
    ...
    refute_match "kaios gate --config workflows/research.json", missing_custom_verify
    ...
    assert_match '"kaios doctor --fix --dry-run --ci --force"', invalid_doctor
    assert_match '"kaios doctor --fix --ci --force"', invalid_doctor
    assert_match '"id": "repair-project"', invalid_doctor
    ...
    assert_match '"fixFirst": {', invalid_bug_report
    assert_match '"command": "kaios doctor --fix --dry-run --ci --force"', invalid_bug_report
    assert_match '"kaios config validate --config kaios.json --json"', invalid_bug_report
    assert_match '"kaios doctor --fix --dry-run --ci --force"', invalid_bug_report
    assert_match '"kaios doctor --fix --ci --force"', invalid_bug_report
    assert_match '"id": "repair-project"', invalid_bug_report
    ...
    assert_match "fix_first: kaios doctor --fix --dry-run --ci --force", invalid_verify
    assert_match "kaios doctor --fix --ci --force", invalid_verify
    assert_match "kaios setup --ci --force", invalid_verify
    refute_match "kaios gate --config kaios.json", invalid_verify
    invalid_verify_json = shell_output("cd invalid-config-fixture && #{bin}/kaios verify --evidence --json --force 2>&1", 2)
    assert_match '"id": "repair-project"', invalid_verify_json
    assert_match '"command": "kaios doctor --fix --dry-run --ci --force"', invalid_verify_json
    invalid_summary = shell_output(
      "cd invalid-config-fixture && #{bin}/kaios gate --summary-out artifacts/gate-summary.md 2>&1",
      2,
    )
    invalid_summary_text = (testpath/"invalid-config-fixture/artifacts/gate-summary.md").read
    assert_match "status: failed", invalid_summary
    assert_match "summary:", invalid_summary
    assert_match "## KAI OS gate", invalid_summary_text
    assert_match "### Verdict", invalid_summary_text
    ...
    assert_match "kaios doctor --fix --dry-run --ci --force", invalid_summary_text
    ...
    (testpath/"doctor-fix-fixture").mkpath
    doctor_fix_plan = shell_output("cd doctor-fix-fixture && #{bin}/kaios doctor --fix --dry-run --ci")
    assert_match "KAI OS doctor fix", doctor_fix_plan
    assert_match "schema: kaios.doctor-fix/v1", doctor_fix_plan
    assert_match "status: planned", doctor_fix_plan
    assert_match "dry_run: true", doctor_fix_plan
    assert_match "config: created (kaios.json)", doctor_fix_plan
    assert_match "ci: created (.github/workflows/kaios.yml)", doctor_fix_plan
    assert_match "kaios doctor --fix --ci", doctor_fix_plan
    refute_predicate testpath/"doctor-fix-fixture/kaios.json", :exist?
    refute_predicate testpath/"doctor-fix-fixture/.github/workflows/kaios.yml", :exist?
    doctor_fix_plan_json = shell_output("cd doctor-fix-fixture && #{bin}/kaios doctor --fix --dry-run --json")
    assert_match '"schema": "kaios.doctor-fix/v1"', doctor_fix_plan_json
    assert_match '"status": "planned"', doctor_fix_plan_json
    assert_match '"dryRun": true', doctor_fix_plan_json
    assert_match '"setup": null', doctor_fix_plan_json
    assert_match '"after": null', doctor_fix_plan_json
    doctor_fix = shell_output("cd doctor-fix-fixture && #{bin}/kaios doctor --fix")
    assert_match "KAI OS doctor fix", doctor_fix
    assert_match "status: fixed", doctor_fix
    assert_match "setup: ready config=created ci=skipped", doctor_fix
    assert_match "after: ready", doctor_fix
    assert_predicate testpath/"doctor-fix-fixture/kaios.json", :exist?
    refute_predicate testpath/"doctor-fix-fixture/.github/workflows/kaios.yml", :exist?
    doctor_fix_validate = shell_output("cd doctor-fix-fixture && #{bin}/kaios config validate --json")
    assert_match '"valid": true', doctor_fix_validate
    (testpath/"doctor-fix-fixture").rmtree
    ...
    assert_match '"kaios gate --config workflows/research.json"', custom_doctor
    ...
    assert_match '"kaios gate --config workflows/research.json"', custom_bug_report
    ...
    assert_match "Need the next move?", help
    assert_match "kaios next", help
    assert_match "Quick start (one command):", help
    assert_match "kaios quickstart", help
    assert_match "Manual path (3 steps):", help
    assert_match "kaios demo", help
    assert_match "kaios setup --ci", help
    assert_match "kaios gate", help
    assert_match "Common aliases:", help
    assert_match "kaios start [--no-ci]", help
    assert_match "kaios status", help
    assert_match "kaios ls", help

    empty_help = shell_output("#{bin}/kaios")
    assert_match "Need the next move?", empty_help
    assert_match "kaios next", empty_help
    assert_match "Quick start (one command):", empty_help
    assert_match "kaios quickstart", empty_help
    assert_match "kaios gate", empty_help

    next_help = shell_output("#{bin}/kaios help next")
    assert_match "Usage: kaios next", next_help
    assert_match "single best next command", next_help
    assert_match "read-only", next_help
    assert_match "kaios.next/v1", next_help
    ...
    analyse_help = shell_output("#{bin}/kaios help analyse")
    assert_match "Usage: kaios analyze", analyse_help
    ...
    assert_match "kaios gate", empty_runs
    ...
    assert_match "status: ready", quickstart
    ...
    (testpath/"alias-fixture").mkpath
    alias_start = shell_output("cd alias-fixture && #{bin}/kaios start --no-ci")
    assert_match "KAI OS quickstart", alias_start
    assert_match "setup: ready config=created ci=skipped", alias_start
    assert_predicate testpath/"alias-fixture/kaios.json", :exist?
    refute_predicate testpath/"alias-fixture/.github/workflows/kaios.yml", :exist?
    alias_status = shell_output("cd alias-fixture && #{bin}/kaios status")
    ...
    assert_match '"version": "0.1.84"', capsule_json_file
    ...
    assert_match "differences:", changed_evidence
    assert_match "changes:", changed_evidence
    assert_match "task:", changed_evidence
    evidence_changed_json_path = testpath/"artifacts/evidence-changed-json.capsule.json"
    changed_evidence_json = shell_output(
      "#{bin}/kaios evidence #{diff_changed_id} --out #{evidence_changed_json_path} --baseline #{diff_baseline_path} --check --json 2>&1",
      1,
    )
    assert_match '"schema": "kaios.evidence/v1"', changed_evidence_json
    assert_match '"status": "different"', changed_evidence_json
    assert_match '"changes": [', changed_evidence_json
    assert_match '"field": "task"', changed_evidence_json
    runs_dir = testpath/".kaios/runs"
    ...
    assert_match "## Fix First", bug_report
    assert_match "kaios doctor --fix --dry-run --ci", bug_report
    ...
    assert_match '"fixFirst": {', bug_report_json
    assert_match '"id": "repair-project"', bug_report_json
    assert_match '"command": "kaios doctor --fix --dry-run --ci"', bug_report_json
    ...
    assert_match "## Fix First", (testpath/"artifacts/kaios-bug-report.md").read
    ...
    assert_match "--fix", doctor_help
    assert_match "--dry-run", doctor_help
    assert_match "kaios doctor --fix --dry-run", doctor_help
    assert_match "kaios doctor --fix --ci", doctor_help
    assert_match "kaios doctor --json", doctor_help
    assert_match "kaios.doctor/v1", doctor_help
    assert_match "kaios.doctor-fix/v1", doctor_help
    ...
    gate_help = shell_output("#{bin}/kaios help gate")
    assert_match "Usage: kaios gate", gate_help
    assert_match "production Agent Gate", gate_help
    assert_match "verify --evidence --force", gate_help
    assert_match "--summary-out", gate_help
    assert_match "GITHUB_STEP_SUMMARY", gate_help
    assert_match "kaios.verify/v1", gate_help
    ...
    assert_match "--summary-out", verify_help
    ...
    typo = shell_output("#{bin}/kaios anaylze . 2>&1", 1)
    assert_match "Unknown command 'anaylze'", typo
    ...
    assert_match '"kaios gate --config retry.json"', retry_validate_json
    ...
    assert_match 'KAIOS_VERSION: "0.1.84"', workflow
    ...
    gate = shell_output("#{bin}/kaios gate")
    assert_match "KAI OS gate", gate
    assert_match "schema: kaios.verify/v1", gate
    assert_match "status: ready", gate
    assert_match "gate_status: ready", gate
    assert_match "evidence: valid (kaios.evidence/v1)", gate
    gate_json = shell_output("#{bin}/kaios gate --json")
    assert_match '"schema": "kaios.verify/v1"', gate_json
    assert_match '"schema": "kaios.evidence/v1"', gate_json
    assert_match '"diagnosis": {', gate_json
    assert_match '"status": "ready"', gate_json
    assert_match '"fixFirst": null', gate_json
    assert_match '"status": "valid"', gate_json
    gate_summary_path = testpath/"artifacts/gate-summary.md"
    gate_summary = shell_output("#{bin}/kaios gate --summary-out #{gate_summary_path}")
    assert_match "summary: #{gate_summary_path}", gate_summary
    assert_match "## KAI OS gate", gate_summary_path.read
    assert_match "### Verdict", gate_summary_path.read
    assert_match "Ready. Agent Gate passed and the run is inspectable.", gate_summary_path.read
    assert_match "| Agent Gate | `ready` |", gate_summary_path.read
    assert_match "| Readiness | `ready` |", gate_summary_path.read
    assert_match "| Process metrics | `recorded` |", gate_summary_path.read
    assert_match "| Evidence | `valid` |", gate_summary_path.read
    gate_summary_json_path = testpath/"artifacts/gate-summary-json.md"
    gate_summary_json = shell_output("#{bin}/kaios gate --json --summary-out #{gate_summary_json_path}")
    assert_match '"schema": "kaios.verify/v1"', gate_summary_json
    refute_match "summary:", gate_summary_json
    assert_match "## KAI OS gate", gate_summary_json_path.read
    ...
    assert_match '"diagnosis": {', verify_evidence_json
    assert_match '"status": "ready"', verify_evidence_json
    assert_match '"fixFirst": null', verify_evidence_json
    assert_match '"status": "valid"', verify_evidence_json
    assert_match '"schema": "kaios.run-capsule/v1"', verify_custom_evidence_path.read
    verify_diff_summary_path = testpath/"artifacts/verify-diff-summary.md"
    verify_evidence_diff = shell_output(
      "#{bin}/kaios verify --evidence --baseline #{diff_baseline_path} --check --summary-out #{verify_diff_summary_path} --force 2>&1",
      1,
    )
    assert_match "schema: kaios.verify/v1", verify_evidence_diff
    assert_match "status: ready", verify_evidence_diff
    assert_match "gate_status: different", verify_evidence_diff
    assert_match "evidence: different (kaios.evidence/v1)", verify_evidence_diff
    assert_match "diff: different", verify_evidence_diff
    assert_match "why:", verify_evidence_diff
    assert_match "baseline diff:", verify_evidence_diff
    assert_match "fix_first: kaios diff", verify_evidence_diff
    assert_match "### What Changed", verify_diff_summary_path.read
    assert_match "| `task` |", verify_diff_summary_path.read
    assert_match "verify KAI OS project workflow", verify_diff_summary_path.read
    ...
    assert_match "kaios gate --config kaios.json", configured_doctor
    refute_match "kaios setup --ci", configured_doctor
    configured_bug_report_json = shell_output("#{bin}/kaios bug-report --json")
    assert_match '"fixFirst": null', configured_bug_report_json
    assert_match '"kaios gate --config kaios.json"', configured_bug_report_json
    assert_match '"id": "verify-project"', configured_bug_report_json
    refute_match '"kaios setup --ci"', configured_bug_report_json
    configured_next_json = shell_output("#{bin}/kaios next --json")
    assert_match '"schema": "kaios.next/v1"', configured_next_json
    assert_match '"status": "ready"', configured_next_json
    assert_match '"id": "show-processes"', configured_next_json
    assert_match '"command": "kaios ps"', configured_next_json
    assert_match '"fixFirst": null', configured_next_json
    ...
    assert_match 'KAIOS_VERSION: "0.1.84"', setup_workflow
    ...
    assert_match "kaios gate --config workflows/research.json", custom_setup
    custom_workflow = (testpath/"custom-config-fixture/.github/workflows/kaios.yml").read
    assert_match %q(kaios gate --config 'workflows/research.json' --summary-out "$GITHUB_STEP_SUMMARY" --json | tee artifacts/kaios-verify.json), custom_workflow
